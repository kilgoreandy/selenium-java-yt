...
public class mathFormulas {
	public float functionOne(int x) {return (2*x)+3;	}
	public float functionTwo(int x) {return (x*x) + (3*x) + 9;	}
	public float functionThree(int x) {return (x*x*x) + (4*x) + 7;	}
	public double findHypotenuse(int x, int y) {
		float z = ((x*x)+ (y*y));
		double c = Math.sqrt(z);
		return c;	}
	public float squarePerimeter(int x) {return 4*x;	}
	public float squareArea(int x) {return x*x;	}
	public float rectArea(int x, int y) {return x*y;	}
	public double triangleArea(int x, int y) {return (0.5 * (x*y));	}
